const createForm = function createForm(obj) {
  ...
  switch (type) {
    ...
    case 'dropdownConnect':
      ...
      for (let i = 0; i < dropdownConnectOptions.length; i += 1) {
        el += `<option value="${dropdownConnectOptions[i].name}">${dropdownConnectOptions[i].name}</option>`;
      }
      el += '</select></div>';
      break;
    case 'image': {
